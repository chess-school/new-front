...
// Новая страница, работающая только с ID из URL
export const ProfilePageById: React.FC = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { userId } = useParams<{ userId: string }>(); // Единственный источник ID
  ...
  // Состояния. Будут всегда "чистыми" благодаря key в роутере
  ...
  // Data Fetching: Упрощенная логика, зависит только от userId
  useEffect(() => {
    if (authLoading) return; // Ждем готовности AuthContext
    ...
    const fetchData = async () => {
      setLoading({ profile: true, coach: false, schedule: false }); // Начинаем загрузку
      try {
  ...
  // Загрузка зависимых данных (без изменений)
  useEffect(() => {
    if (profileUser) {
  ...
  // Handlers (логика остается той же, просто берет `profileUser` из состояния)
